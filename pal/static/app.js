...
async function generateBrief() {
    ...
    try {
        // Fire brief + overall in parallel (sector gaps moved to Action Items)
        const [briefRes, overallRes] = await Promise.all([
            fetch(`${API}/api/brief`),
            fetch(`${API}/api/overall_brief`)
        ]);

        const [briefData, overallData] = await Promise.all([
            briefRes.json(), overallRes.json()
        ]);

        // Render overall brief hero
        const overallCards = [];
        if (overallData && overallData.bluf) {
            overallCards.push(buildOverallBriefCard(overallData));
        }

        if (overallCards.length > 0) {
            overallWrap.innerHTML = overallCards.join('');
        } else {
            overallWrap.classList.add('hidden');
        }
...
function buildBriefCard(item, idx) {
    ...
    let upDownIcon = '';
    if (movement > 0) {
        upDownIcon = `<span style="color:var(--bullish); font-size:1.1rem; margin-left:4px;">▲ ${movement.toFixed(2)}%</span>`;
    } else if (movement < 0) {
        upDownIcon = `<span style="color:var(--bearish); font-size:1.1rem; margin-left:4px;">▼ ${Math.abs(movement).toFixed(2)}%</span>`;
    }
    ...
    // Source Tracking toggle
    const articles = item.raw_articles || [];
    let sourceLinkage = '';
    if (articles.length > 0) {
        const links = articles.map(a => `<li style="margin-bottom:6px"><a href="${a.url || a.link || '#'}" target="_blank" style="color:var(--brand); text-decoration:none;">${a.title}</a> <span style="color:var(--text-3); font-size: 0.8rem">- ${a.source || a.publisher || 'Unknown'}</span></li>`).join('');
        sourceLinkage = `<details style="margin-top: 15px; background:var(--bg-2); padding: 14px 16px; border-radius:6px; border:1px solid var(--border)">
            <summary style="cursor:pointer; font-weight:600; font-size:0.85rem; color:var(--text-2);">News Source Tracking (${articles.length})</summary>
            <ul style="margin: 10px 0 0 0; padding-left: 20px; font-size: 0.85rem;">${links}</ul>
        </details>`;
    }

    card.innerHTML = `
        <div class="brief-card-header ${sentiment}">
            <div class="brief-ticker">${item.ticker}${upDownIcon}</div>
...
function buildSectorGapsCard(data) {
    const gapsList = data.underrepresented_sectors || [];
    let html = `<div class="overall-brief-card" style="margin-top:20px;">
        <div class="overall-brief-meta">
            <span class="overall-tag">Sector Gap Analysis</span>
        </div>
        <div class="overall-brief-headline">Your portfolio is underweight in ${gapsList.join(', ')}.</div>
        <div class="overall-brief-sections">`;

    gapsList.forEach(sector => {
        const port_w = (data.portfolio_weights[sector] || 0) * 100;
        const b_w = (data.benchmark_weights[sector] || 0) * 100;
        html += `<div class="brief-article-section">
            <div class="brief-article-label">Underrepresented: ${sector} (${port_w.toFixed(1)}% vs benchmark ${b_w.toFixed(1)}%)</div>`;
        const nList = data.sector_news[sector] || [];
        if (nList.length === 0) {
            html += `<p class="brief-article-body">No recent news found for this sector ETF.</p>`;
        } else {
            html += `<ul class="bullet-list">`;
            nList.slice(0, 3).forEach(n => {
                html += `<li><a href="${n.link}" target="_blank" style="color:var(--brand)">${n.title}</a> <span style="color:var(--text-3); font-size: 0.8rem">- ${n.publisher}</span></li>`;
            });
            html += `</ul>`;
        }
        html += `</div>`;
...
function buildPortfolioNewsCard(newsData) {
    let hasNews = false;
    let html = `<div class="overall-brief-card" style="margin-top:20px;">
        <div class="overall-brief-meta">
            <span class="overall-tag">YFinance Portfolio News</span>
        </div>
        <div class="overall-brief-sections" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 15px;">`;

    for (const [ticker, arr] of Object.entries(newsData)) {
        if (!arr || arr.length === 0) continue;
        hasNews = true;
        html += `<div style="background:var(--bg-2); padding:10px; border-radius:6px;">
            <strong style="color:var(--text-1);">${ticker}</strong>
            <ul style="margin: 8px 0 0 -20px; font-size: 0.9rem;">`;
        arr.slice(0, 3).forEach(n => {
            html += `<li style="margin-bottom:5px;"><a href="${n.link}" target="_blank" style="color:var(--brand); text-decoration:none;">${n.title}</a> <br><span style="color:var(--text-3); font-size: 0.75rem">${n.publisher}</span></li>`;
        });
        html += `</ul></div>`;
    }
    html += `</div></div>`;
...
async function loadActions() {
    ...
    try {
        // Fetch action items and sector gaps in parallel
        const [actionsRes, gapsRes] = await Promise.all([
            fetch(`${API}/api/actions`),
            fetch(`${API}/api/portfolio/sector-gaps`)
        ]);
        const [actionsData, gapsData] = await Promise.all([
            actionsRes.json(), gapsRes.json()
        ]);
        renderActions(actionsData.items || [], gapsData, cont);
    } catch (e) {
...
function renderActions(items, gapsData, cont) {
    let html = '';
    ...
    // Render Sector Gap Analysis
    if (gapsData && !gapsData.error && gapsData.underrepresented_sectors && gapsData.underrepresented_sectors.length > 0) {
        html += buildSectorGapsCard(gapsData);
    }
