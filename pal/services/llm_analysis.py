...
def _parse_json(raw: str):
    """Strip markdown fences and extract the first JSON object or list."""
    raw = raw.strip().strip("`")
    if raw.lower().startswith("json"):
        raw = raw[4:].strip()
    start = raw.find("{")
    end   = raw.rfind("}") + 1
    # Fallback to look for a JSON array if not an object
...
def analyze_news_batch(holdings: list[dict], news_batch: list[dict],
                       profile: dict, macro_news: list[dict] = None, movements: dict = None) -> list[dict]:
    api_key = os.getenv("FEATHERLESS_API_KEY")
  ...
  "action_signal": "Monitor" or "Review" or "Act"
        ...
        for attempt in range(MAX_RETRIES + 1):
            try:
                ...
                # Unwrap single-element list: LLM sometimes returns [{...}]
                ...
                print(f"[Retry {attempt+1}/{MAX_RETRIES+1}] {ticker}: LLM returned {type(parsed).__name__}, raw output: {raw_content[:200]}")
                parsed = None  # reset for next attempt

            except Exception as e:
                print(f"[Retry {attempt+1}/{MAX_RETRIES+1}] {ticker}: Exception: {e}")
                parsed = None
        ...
        if isinstance(parsed, dict):
            ...
            parsed["raw_articles"] = [n for n in news_batch if n.get("ticker", "").upper() == ticker.upper()]
            parsed["movement"]   = movements.get(ticker, 0.0) if movements else 0.0
            results.append(parsed)
        else:
            print(f"FAILED {ticker}: All {MAX_RETRIES+1} attempts returned non-dict. Falling back to error card.")
  ...
  "portfolio_sentiment": "Bullish" or "Bearish" or "Mixed"
    ...
    try:
        response = client.chat.completions.create(
            model=model,
            messages=[
                {"role": "system", "content": "You are a Chief Investment Officer synthesizing portfolio data. Output raw JSON only."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.0,
            max_tokens=800,
        ...
        if not isinstance(parsed, dict):
            raise ValueError(f"Overall brief parsed JSON mapped to {type(parsed)}")
        return parsed
    except Exception as e:
